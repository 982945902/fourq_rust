...
typedef unsigned uint128_t __attribute__((mode(TI)));
    ...
    (OS_TARGET == OS_WIN && COMPILER == COMPILER_VC)
...
static __inline unsigned int is_digit_lessthan_ct(digit_t x,
                                                  digit_t y) {  // Is x < y?
...
#define MUL128(multiplier, multiplicand, product)              \
  mp_mul((digit_t *)&(multiplier), (digit_t *)&(multiplicand), \
         (digit_t *)&(product), NWORDS_FIELD / 2);

// 128-bit addition, inputs < 2^127
#define ADD128(addend1, addend2, addition)                                  \
  mp_add((digit_t *)(addend1), (digit_t *)(addend2), (digit_t *)(addition), \
         NWORDS_FIELD);

// 128-bit addition with output carry
#define ADC128(addend1, addend2, carry, addition)              \
  (carry) = mp_add((digit_t *)(addend1), (digit_t *)(addend2), \
                   (digit_t *)(addition), NWORDS_FIELD);
...
#define SUBC(borrowIn, minuend, subtrahend, borrowOut, differenceOut) \
  (borrowOut) =                                                       \
      _subborrow_u64((borrowIn), (minuend), (subtrahend), &(differenceOut));
...
#define ADD128(addend1, addend2, addition)                            \
  {                                                                   \
    unsigned char carry =                                             \
        _addcarry_u64(0, (addend1)[0], (addend2)[0], &(addition)[0]); \
...
#define SUB128(minuend, subtrahend, difference)                              \
  {                                                                          \
    unsigned char borrow =                                                   \
        _subborrow_u64(0, (minuend)[0], (subtrahend)[0], &(difference)[0]);  \
...
#elif ((TARGET == TARGET_AMD64 || TARGET == TARGET_ARM64) && \
       OS_TARGET == OS_LINUX)
...
#define ADDC(carryIn, addend1, addend2, carryOut, sumOut)                   \
  {                                                                         \
    uint128_t tempReg =                                                     \
        (uint128_t)(addend1) + (uint128_t)(addend2) + (uint128_t)(carryIn); \
...
#if defined(GENERIC_IMPLEMENTATION)
unsigned int subtract(const digit_t *a, const digit_t *b, digit_t *c,
                      const unsigned int nwords);
#else
unsigned char subtract(const digit_t *a, const digit_t *b, digit_t *c,
                       const unsigned int nwords);
#endif
...
// Conversion from representation (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT), where T =
// Ta*Tb
void R1_to_R2(point_extproj_t P, point_extproj_precomp_t Q);

// Conversion from representation (X,Y,Z,Ta,Tb) to (X+Y,Y-X,Z,T), where T =
// Ta*Tb
...
void eccadd_core(point_extproj_precomp_t P, point_extproj_precomp_t Q,
                 point_extproj_t R);
...
void fixed_window_recode(uint64_t *scalar, unsigned int *digits,
                         unsigned int *sign_masks);
...
// Constant-time table lookup to extract an extended twisted Edwards point
// (X+Y:Y-X:2Z:2T) from the precomputed table
void table_lookup_1x8(point_extproj_precomp_t *table, point_extproj_precomp_t P,
                      unsigned int digit, unsigned int sign_mask);
void table_lookup_1x8_a(point_extproj_precomp_t *table,
                        point_extproj_precomp_t P, unsigned int *digit,
                        unsigned int *sign_mask);

// Modular correction of input coordinates and conversion to representation
// (X,Y,Z,Ta,Tb)
...
void table_lookup_fixed_base(point_precomp_t *table, point_precomp_t P,
                             unsigned int digit, unsigned int sign);

//  Computes the modified LSB-set representation of scalar
void mLSB_set_recode(uint64_t *scalar, unsigned int *digits);

// Generation of the precomputation table used internally by the double scalar
// multiplication function ecc_mul_double()
void ecc_precomp_double(point_extproj_t P, point_extproj_precomp_t *Table,
